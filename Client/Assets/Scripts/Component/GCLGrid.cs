    ...
    public Vector2 GetPos(int index)
    {
        ...
        switch (childAligment)
        {
            case Aligment.LowerLeft:
                if (is3D)
                {
                    ret = new Vector2((columnIndex * offset.x),
                        columnIndex * spacing.x + rowIndex * spacing.y + (columnIndex * offset.y));
                }
                else
                {
                    ret = new Vector3(columnIndex * spacing.x + (rowIndex * offset.x),
                        rowIndex * spacing.y + (columnIndex * offset.y));
                }
                break;
            case Aligment.LowerRight:
                if (is3D)
                {
                    ret = new Vector3((columnIndex * offset.x),
                        columnIndex * -spacing.x + rowIndex * spacing.y + (columnIndex * offset.y));
                }
                else
                {
